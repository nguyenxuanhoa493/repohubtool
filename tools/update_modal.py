...
new_modal_code = """class RetroHubWebModal(BaseModal):
        ...
        if btn_y:
            ...
            def _bg_send():
                try:
                    ...
                    from ..logger import upload_log_to_telegram
                    ip = get_ip()
                    msg = f"🌐 <b>RetroHub Web Manager:</b>\\nhttp://{ip}:8888\\n\\nBao gồm: Quản lý Game, Tải ROM, YouTube, Stream màn hình, Quản lý File"
                    upload_log_to_telegram(note=msg)
                    if self.engine:
                        self.engine.toast("Đã gửi thông tin Web sang Telegram!")
                except Exception as e:
                    if self.engine:
                        self.engine.toast(f"Lỗi gửi Telegram: {e}")
    ...
    def render(self, engine):
        ...
        # 1. Row: Status & IP (Combined, Huge IP)
        cy = my + head_h + 16
        card1_h = 86
        ...
        st_text = "🟢 ĐANG BẬT" if self.is_running else "🔴 ĐANG TẮT"
        if not vi:
            st_text = "🟢 RUNNING" if self.is_running else "🔴 STOPPED"
            
        engine.draw_text(st_text, engine.font_item, cx + 24, cy + card1_h // 2, 255, 255, 255, center_y=True)
        ...
        # IP text huge! (font_huge)
        font_ip = engine.font_huge if self.is_running else engine.font_item
        engine.draw_text(ip_url, font_ip, cx + 240, cy + card1_h // 2, ip_col[0], ip_col[1], ip_col[2], center_y=True)

        # 2. Row: Connect Info & Features Compact
        cy += card1_h + 12
        card2_h = 92
        ...
        val_conn = "Mở Chrome/Safari trên máy tính, ĐT chung Wi-Fi" if vi else "Open Chrome/Safari/Edge on PC/phone on same Wi-Fi"
        engine.draw_text(lbl_conn, engine.font_badge, cx + 18, cy + 26, 185, 210, 245, center_y=True)
        engine.draw_text(val_conn, engine.font_sub, cx + 185, cy + 26, 255, 255, 255, center_y=True)

        lbl_feat = "Chức năng:" if vi else "Features:"
        val_feat = "ROMs, Tải Game, YouTube, Stream màn hình, Quản lý File" if vi else "Manage ROMs, Downloads, YouTube, Stream, Files"
        engine.draw_text(lbl_feat, engine.font_badge, cx + 18, cy + 64, 185, 210, 245, center_y=True)
        engine.draw_text(val_feat, engine.font_sub, cx + 185, cy + 64, 255, 255, 255, center_y=True)
        ...
        fx = engine.draw_footer_btn(fx + 16, fy, foot_h - 2, "Y", "Gửi Telegram" if vi else "Telegram", (0, 180, 255), is_dark_btn=False)

        engine.draw_footer_btn(mx + mw - 145, fy, foot_h - 2, "B", "Đóng" if vi else "Close", (200, 210, 225), is_dark_btn=False)
"""
...
print("Replaced RetroHubWebModal!")
