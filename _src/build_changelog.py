...
RELEASES = [
    ("2.39", "2026-09-19",
     ("Fix the download queue, the OTA update loop, Switch emulator core and the blank Storage screen; add the YouTube watch page and queue; stop the game detail panel from overflowing",
      "Sửa hàng chờ tải, vòng lặp cập nhật OTA, Switch emulator core và màn Storage trắng; thêm trang xem & queue YouTube; màn chi tiết game hết tràn panel phải"),
     [("Download queue: the first download used to hold the result slot forever, so every later download only queued. Closing the modal now releases the slot and cancelling removes a queued item.",
       "Hàng chờ tải: lần tải đầu giữ slot kết quả mãi mãi nên mọi lần sau chỉ xếp hàng. Đóng modal này trả slot và hủy được cả mục đang chờ."),
      ("Detail screen vs installed copy: the store keeps the .zip name while Roms keeps the extracted ROM name, so the app thought a game was missing after installing it. Matching is now by base name, for companions too.",
       "Màn chi tiết khớp bản đã cài: kho ghi tên .zip còn Roms giữ tên ROM đã bung nên app tưởng game chưa có. Nay khớp theo tên gốc, kể cả file phụ."),
      ("Safer zip extraction: files extract into a staging folder, free space is checked first, and the directory tree inside the zip is preserved instead of flattening every ROM into one folder.",
       "Giải nén zip an toàn hơn: giải nén vào thư mục tạm, kiểm tra dung lượng trước, giữ nguyên cấu trúc thư mục trong zip thay vì dồn hết ROM ra một chỗ."),
      ("OTA popup loop solved: the J2ME keypad config and settings.json were shipped and also written by the app, so pending files never converged and every launch offered the same update again. Both left the payload.",
       "Hết vòng lặp popup OTA: cấu hình bàn phím J2ME và settings.json vừa nằm trong gói phát hành vừa bị app ghi đè nên danh sách file chờ không bao giờ rỗng. Cả hai đã bị loại khỏi payload."),
      ("Update UI: the modal now says what it is actually updating (game catalog, J2ME emulator) instead of claiming a new version, and logs a diagnostic line after the update.",
       "UI cập nhật: modal nói đúng bản chất (kho game, giả lập J2ME) thay vì báo bản mới, và ghi log chẩn đoán sau khi cập nhật."),
      ("Utilities: Switch emulator core works again (the item had no handler), Storage & SD card logs and falls back to a placeholder row instead of a blank screen, and the core picker uses ASCII arrows for devices whose font lacks the glyphs.",
       "Tiện ích: Switch emulator core chạy lại (mục này trước không có handler), Storage & thẻ nhớ ghi log và có dòng dự phòng thay vì trắng màn, core picker dùng mũi tên ASCII cho máy thiếu glyph."),
      ("YouTube: watch page with queue, history, playlist and resume; a loading popup appears when playback hands over to RetroArch, including when playing from the queue; related videos load with thumbnails; Save to favorites turns yellow once saved. Audio-only was dropped because RetroArch cannot honour it.",
       "YouTube: watch page kèm queue, lịch sử, playlist và resume; có popup chờ khi chuyển sang RetroArch, kể cả khi phát từ queue; video liên quan tải được kèm thumbnail; nút Save to favorites đổi vàng khi đã lưu. Bỏ audio-only vì RetroArch không hỗ trợ được."),
      ("Detail & download screen no longer overflows: long game names and file names are cut by pixel width, so text stays inside the cards instead of spilling to the screen edge; long action labels are bounded to their tile.",
       "Màn chi tiết & tải game hết tràn: tên game và tên file dài được cắt theo pixel nên chữ nằm trong card thay vì tràn ra sát lề màn hình; nhãn nút dài bị giới hạn theo ô."),
      ("Boxart download deadline: fetching a cover image now has a 30s budget for the whole transfer and a 12MB ceiling. A slow mirror that trickles bytes forever is skipped, and a heavy image is refused instead of stalling the scraper; the old code only limited each socket read, so a trickling server could hold the download open indefinitely.",
       "Hạn giờ tải ảnh bìa: mỗi lần tải có ngân sách 30s cho cả lần truyền và trần 12MB. Máy chủ nhỏ giọt từng byte sẽ bị bỏ qua, ảnh quá nặng bị từ chối thay vì treo tiến trình cào ảnh; code cũ chỉ giới hạn từng lần đọc socket nên server nhỏ giọt có thể giữ lần tải mãi."),
      ("Extraction diagnosis fixed: the 7-Zip listing parser only accepted LF output and a ten-dash separator, so on Windows and with newer 7-Zip builds it failed and the app blamed a healthy archive as a damaged download. Errors are now separated into \"download is damaged/incomplete\", \"archive holds no ROM\" and \"ROM is an ECM-packed CD image\", archives are verified right after download so a bad mirror is skipped instead of surfacing later as an extraction failure, stale temp files left by interrupted downloads are purged, and every download/extraction failure is written to RetroHub/logs/tai-loi.log with the byte count that was received against the size the server advertised.",
       "Chan doán lỗi giải nén: bộ đọc danh sách của 7-Zip chỉ nhận LF và dòng ngăn cách đúng 10 dấu gạch, nên trên Windows hay với bản 7-Zip mới là hỏng và app đổ lỗi cho một file nén lành lặn. Nay phân biệt rõ \"file tải về hỏng/thiếu\", \"nén không chứa ROM\" và \"ROM là ảnh CD đã nén ECM\", kiểm tra archive ngay sau khi tải để bỏ qua nguồn lỗi thay vì để nó nổ ra ở bước giải nén, dọn file tạm do lần tải bị ngắt, và ghi mọi lỗi tải/giải nén vào RetroHub/logs/tai-loi.log kèm số byte đã nhận so với kích thước server báo."),
      ("Multi-language and misc: the newly added screens are fully translated, the store search header reads \"Search results for ...\", the exit dialog no longer hangs, the web logger works again, desktop gets a windowed mode, and the repo gains AGENTS.md, a hash guard and self-tests.",
       "Đa ngôn ngữ và phần còn lại: các màn mới đã dịch đủ, header tìm kiếm hiện \"Kết quả tìm kiếm cho ...\", hộp thoại thoát app không còn treo, web logger hoạt động lại, desktop có chế độ cửa sổ, repo thêm AGENTS.md, hash guard và selftest.")]),

    ("2.38", "2026-09-19",
     ("Fix SSL certificate verification and add cURL fallback for online emulator downloads",
      "Khắc phục triệt để lỗi chứng chỉ SSL và bổ sung cURL fallback khi tải giả lập online"),
     [("Online emulator downloads retry with an unverified SSL context and fall back to cURL when the bundled Python cannot validate the chain on the device.",
       "Tải giả lập online thử lại với ngữ cảnh SSL không xác thực và chuyển sang cURL khi Python trên máy không kiểm tra được chuỗi chứng chỉ.")]),
    ...
    ("2.28", "2026-09-17",
     ...
     [("RetroHub AI Web Assistant & Shell Command Execution: Integrated intelligent AI assistant into the Web Game Manager interface (Port 8888). Features interactive chat, retro gaming recommendations, real-time device system prompt inspection, terminal log search, wake lock management, and direct Linux shell command execution on TrimUI devices.",
       "Trợ lý RetroHub AI & Thực thi lệnh Web: Tích hợp trợ lý AI thông minh trực tiếp trên giao diện Web Game Manager (Cổng 8888). Hỗ trợ trò chuyện tương tác, gợi ý game retro theo sở thích, tra cứu System Prompt thiết bị, tìm kiếm log thời gian thực, quản lý Wake Lock và cho phép AI thực thi lệnh shell Linux trực tiếp trên máy TrimUI."),
